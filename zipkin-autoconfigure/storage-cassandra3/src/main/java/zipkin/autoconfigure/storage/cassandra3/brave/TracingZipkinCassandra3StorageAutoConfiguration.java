...
public class TracingZipkinCassandra3StorageAutoConfiguration {
  ...
  @Bean SessionFactory tracingSessionFactory() {
    return storage -> TracingSession.create(CassandraClientTracing.newBuilder(tracing)
        .sampler(CassandraClientSampler.NEVER_SAMPLE).build(), delegate.create(storage));
  }
}
